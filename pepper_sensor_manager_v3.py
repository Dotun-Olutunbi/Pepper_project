...
import tty
# import os
from naoqi_callbacks_v3 import NaoqiEventHandler
...
def verify_services(session):
    try:
        # Test basic services
        memory = session.service("ALMemory")
        print("✓ ALMemory service accessible")
        
        # Test motion services
        motion = session.service("ALMotion")
        print("✓ ALMotion service accessible")
        
        # Test audio services
        audio = session.service("ALAudioDevice")
        print("✓ ALAudioDevice service accessible")
        
        # Test face detection
        face_detection = session.service("ALFaceDetection")
        print("✓ ALFaceDetection service accessible")
        
        # Test awareness
        awareness = session.service("ALBasicAwareness")
        print("✓ ALBasicAwareness service accessible")
...
def main():
    ...
    try:
        ...
        # Verify all services first
        if not verify_services(session):
            print("❌ Service verification failed. Please check robot status.")
            return

        # Setup session to listen for connections
        ...
        session.registerService("NaoqiEventHandler", handler)
        ...
        asr_service = setup_speech_recognition(session, memory_service, "NaoqiEventHandler")
        if asr_service:
        ...
        # Setup other sensors
        setup_tactile_sensors(memory_service, "NaoqiEventHandler")
        setup_bumper_sensors(memory_service, "NaoqiEventHandler")
